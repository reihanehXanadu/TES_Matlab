...
function [varargout,F] = Calibrate(S,data,varargin)
clear F
...
F=zeros(S.RecordsPerBuffer*S.buffersPerAcquisition,NumberOfChannelsToProcess);
...
hold off
 ...
 PC1(:,i)=PC(:,Index_maxL);
 F(:,i)=sum(X.*repmat(PC1(:,i)',S.RecordsPerBuffer*S.buffersPerAcquisition,1),2);
